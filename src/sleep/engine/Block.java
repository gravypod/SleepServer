 ...
 * 
 * <p>
 * The variable value would contain the return value after the block was
 * executed. It is recommended blocks only be run using
 * {@linkplain sleep.runtime.SleepUtils#runCode(Block, ScriptEnvironment)
 * SleepUtils.runCode()} as there is a little bit of synchronization and cleanup
 * that has to be done prior to and after executing a block of code.
 * </p>
 * 
...
public class Block implements Serializable {
     ...
     * 
     */
	private static final long serialVersionUID = -6469549406898888342L;
	...
	public Block(final String _src) {
	...
	public String toString(final String prefix) {
		...
		final StringBuffer tempz = new StringBuffer();
		Step temp = first;
		while(temp != null) {
	...
	/**
	 * Returns a string representation of the Abstract Syntax Tree (AST). An AST
	 * represents how the sleep parser interpreted a script string
	 */
	@Override
	 ...
	 * returns -1 if no code is in this block (unlikely)
	 */
	...
	/**
	 * Returns an approximate range of line numbers for the steps in this block
	 * object. Useful for formatting error messages in script warnings and such.
	 */
	public String getApproximateLineRange() {
	
		final int low = getLowLineNumber();
		final int high = getHighLineNumber();
	...
	/**
	 * Returns a string representation of where in the source code this block
	 * originated from
	 */
	...
	/**
	 * evaluates this block of code. please note that if the block has a return
	 * statement and the method clearReturn() is not called on the corresponding
	 * script environment chaos will ensue. use SleepUtils.runCode() to safely
	 * execute a block of code. don't call this method yourself. okay?
	 */
	public Scalar evaluate(final ScriptEnvironment environment) {
	
		return evaluate(environment, first);
	}
	
	/** handle an exception raised by a script / java object */
	private void handleException(final ScriptEnvironment environment) {
	
		if (environment.isResponsible(this)) {
			final Block doit = environment.getExceptionHandler();
	...
	private void cleanupEnvironment(final ScriptEnvironment environment) {
	 ...
	 * execute a block of code. don't call this method yourself. okay?
	 */
	public Scalar evaluate(final ScriptEnvironment environment, final Step start) {

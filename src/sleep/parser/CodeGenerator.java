...
/**
 * Generates code for the parser. The main thing here developers might concern
 * themselves with is the installEscapeConstant function
 */
public class CodeGenerator implements ParserConstants {
	
	protected Block CURRENT_BLOCK;
	
	protected Stack<Block> BACKUP_BLOCKS;
	
	protected GeneratedSteps factory; /* allow specification of a factory for generating steps */
	
	protected Parser parser;
	
	protected static HashMap<String, String> escape_constants = new HashMap<String, String>();
	...
	public static void installEscapeConstant(final char c, final String value) {
	
		CodeGenerator.escape_constants.put(c + "", value);
	}
	...
	public void add(final Step n, final Token token) {
	...
	public Block restore() {
		...
		final Block temp = CURRENT_BLOCK;
		CURRENT_BLOCK = BACKUP_BLOCKS.pop();
		return temp;
	}
	
	public CodeGenerator(final Parser _parser, final GeneratedSteps _factory) {
	
		parser = _parser;
		factory = _factory != null ? _factory : new GeneratedSteps();
		
		CURRENT_BLOCK = new Block(parser.getName());
		BACKUP_BLOCKS = new Stack<Block>();
	}
	
	public CodeGenerator(final Parser _parser) {
	
		this(_parser, null);
	}
	
	public Check parsePredicate(final Token data) {
	
		/**
		 * send the data string through the parser pipeline - errors earlier in
		 * the pipeline are assumed to not exist as they would have been found
		 * the first time we processed it as a block
		 */
		final Statement allData = TokenParser.ParsePredicate(parser, LexicalAnalyzer.GroupBlockTokens(parser, new StringIterator(data.toString(), data.getHint())));
		
		return parsePredicate(allData);
	}
	
	public Check parsePredicate(final Statement parsePred) {
	
		final Token[] tokens = parsePred.getTokens();
		final String[] strings = parsePred.getStrings();
		
		//Step atom;
		Check tempc;
		//Block backup, a, b;
		Stack<Token> queue; // locals is a stack created at parse time but used by the operators for keeping track of stuff.. yeah
		...
		switch(parsePred.getType()) {
			...
			case PRED_AND:
				tempc = null;
				queue = new Stack<Token>();
				
				for (final Token token : tokens) {
					queue.push(token);
				}
				while(!queue.isEmpty()) {
					final Token t = queue.pop();
					if (!t.toString().equals("&&")) {
						final Check oldtemp = tempc;
	...
	public void parseObject(final Token data) {
	
		final Statement stmt = TokenParser.ParseObject(parser, LexicalAnalyzer.GroupExpressionIndexTokens(parser, new StringIterator(data.toString(), data.getHint())));
	...
	public void parseObject(final Statement datum) {
	
		Step atom;
		
		final String[] strings = datum.getStrings(); // was "temp"
		final Token[] tokens = datum.getTokens();
	...
	public void parseBlock(final Token data) {
		 ...
		 * the first time we processed it as a block
		 */
		final LinkedList<Statement> allData = TokenParser.ParseBlocks(parser, LexicalAnalyzer.GroupBlockTokens(parser, new StringIterator(data.toString(), data.getHint())));
	...
	public void parseBlock(final LinkedList<Statement> data) {
	
		final Iterator<Statement> i = data.iterator();
		while(i.hasNext()) {
			parse(i.next());
		}
	}
	
	public List<Statement> parseIdea(final Token data) {
	
		final LinkedList<Statement> allData = TokenParser.ParseIdea(parser, LexicalAnalyzer.GroupBlockTokens(parser, new StringIterator(data.toString(), data.getHint())));
		
		if (parser.hasErrors()) {
			return null;
		}
		
		final Iterator<Statement> i = allData.iterator();
		while(i.hasNext()) {
			parse(i.next());
		}
	...
	public void parse(final Statement datum) {
		...
		switch(datum.getType()) {
			...
			case IDEA_HASH_PAIR:
				...
				// parse B
				//
				final List<Statement> valuez = parseIdea(tokens[2]);
				
				/* a bit of error checking to guard against a common error */
				final Iterator<Statement> iz = valuez.iterator();
				while(iz.hasNext()) {
					final Statement t = iz.next();
			...
			case IDEA_LITERAL: // implemented                   
				...
				for (int x = 0; x < sb.length(); x++) {
					if (sb.charAt(x) == '\\' && x + 1 < sb.length()) {
						final char tempc = sb.charAt(x + 1);
			...
			case IDEA_BOOLEAN: // implemented
				...
				break;
			case IDEA_CLASS:
				final Class<?> claz = parser.findImportedClass(strings[0].substring(1));
			...
			case IDEA_STRING: // implemented -- parsed literals, one of my favorite features in sleep
				...
				/**
				 * create a frame, we assume the PLiteral machine will destroy
				 * it
				 */
				atom = factory.CreateFrame();
				add(atom, tokens[0]);
				...
				String varname,
				align; // some temp vars we'll use later...
				
				StringBuffer d = new StringBuffer(); // the string buffer where we will dump our results.
				
				ll = new LinkedList();
				final StringIterator si = new StringIterator(ParserUtilities.extract(strings[0]), tokens[0].getHint());
				...
				while(si.hasNext()) {
					...
					if (current == '\\' && si.hasNext()) {
						...
						if (CodeGenerator.escape_constants.containsKey(mutilate)) {
							d.append(CodeGenerator.escape_constants.get(mutilate));
						} else if (current == 'u') {
							if (!si.hasNext(4)) {
								...
								try {
									final int codepoint = Integer.parseInt(mutilate, 16);
						...
						final String[] ops = LexicalAnalyzer.CreateTerms(parser, new StringIterator(d.toString(), si.getLineNumber())).getStrings();
			...
			case IDEA_FUNC: // implemented 
				final TokenList funcParms = LexicalAnalyzer.CreateTerms(parser, new StringIterator(strings[0], tokens[0].getHint()));
				...
				if ((strings[0].equals("&iff") || strings[0].equals("&?")) && tokens.length > 1) {
					final TokenList terms = ParserUtilities.groupByParameterTerm(parser, ParserUtilities.extract(tokens[1]));
					final Token[] termsAr = terms.getTokens();
			...
			case EXPR_ASSIGNMENT_T_OP:
				...
				final TokenList terms2 = ParserUtilities.groupByParameterTerm(parser, ParserUtilities.extract(tokens[0]));
				final Token[] termsAr2 = terms2.getTokens();
				
				for (final Token element : termsAr2) {
					parseIdea(element);
				}
			...
			case EXPR_FOR:
				...
				final Token extracted_terms[] = ParserUtilities.groupByBlockTerm(parser, ParserUtilities.extract(tokens[1])).getTokens();
				...
				final TokenList initial_terms = ParserUtilities.groupByParameterTerm(parser, extracted_terms[0]);
				...
				if (extracted_terms.length == 3) {
					...
					final TokenList final_terms = ParserUtilities.groupByParameterTerm(parser, extracted_terms[2]);
			...
			case EXPR_ASSERT:
				...
				final Token assert_terms[] = ParserUtilities.groupByMessageTerm(parser, tokens[1]).getTokens();
	...
	public void parseParameters(final Token token) {
	
		final TokenList terms = ParserUtilities.groupByParameterTerm(parser, token);
		final Token[] termsAr = terms.getTokens();
